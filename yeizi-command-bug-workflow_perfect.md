...
**可视化参考文件**：[bug-workflow-final.excalidraw](bug-workflow-final.excalidraw)

```mermaid
flowchart TD
    classDef inputNode fill:#e1f5fe,stroke:#0277bd,stroke-width:2px
    classDef processNode fill:#f3e5f5,stroke:#6a1b9a,stroke-width:2px
    ...
    subgraph 输入层
        A[用户提交 BUG 现象]:::inputNode
    end

    subgraph 问题识别层
        B[轻量解析 BUG 与修复目标]:::processNode
        C[定向感知相关环境]:::processNode
        D{感知是否足够支持进一步判断?}:::decisionNode
        C1[扩展感知边界]:::processNode
        E[完整解析问题和修复目标]:::processNode
    end

    subgraph 方案生成层[第 5 步：两辩一裁协同模式]
        direction LR
        F1[Advocate 提案智能体]:::agentNode
        F2[Skeptic 辩驳智能体]:::agentNode
        F3[Arbiter 仲裁智能体]:::agentNode
        F4{Skeptic 是否仍有新的实质性反驳?}:::decisionNode
...
- 实线箭头：流程执行顺序
- 虚线箭头：数据传入依赖
- 节点颜色：蓝=输入，紫=处理，绿=智能体，橙=判断，红=输出
...
循环退出条件：Skeptic 没有新的实质性反驳意见，或 Arbiter 判定反驳已被充分回应。

群体循环执行期间，主智能体不干预，等待结果回传。

#### 第三阶段：结果回传主智能体

子智能体群体执行完毕后，将完整讨论记录统一回传给主智能体。

主智能体读取回传的完整讨论记录，总结并给出本步的最终解决方案。
...
- 完整代码修改内容
- 本步所有测试结果（包括每轮循环的测试记录）

### 设立原因

把开发和测试分开，可以让每次修复后都有即时反馈，避免一次性改完再测发现一堆问题再回头改。

开发测试只做小规模测试（代码审计、白盒测试），不做大规模集成测试，大规模集成测试放到第 8 步。
...
- 失败时必须明确区分是方案问题还是代码问题，对应反馈到正确的步骤
...
| 生成问题解决方案 | Advocate 提方案，Skeptic 反驳，Arbiter 裁决 | 跳过裁决直接进入实现，或三方提前对齐结论 |
| 可行性分析 | 结合第 4 步问题定义和第 5 步方案，分析实施可行性 | 跳过可行性分析直接进入实施 |
| 实施修复 | 执行最小完整修复 | 混入无关改动 |
| 验证结果 | 用证据确认修复是否成立 | 未验证就宣布成功 |
| 输出结果 | 向用户交付最终解决方案 | 报告未验证的内容 |
